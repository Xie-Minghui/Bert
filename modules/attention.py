...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        mixed_query_layer = self.query(hidden_states)  # 对输入使用相应的权重矩阵转化为对应的query张量
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            attention_mask = encoder_attention_mask
        elif past_key_value is not None:  # 如果在解码阶段使用
        ...
        context_layer = torch.matmul(attention_probs, value_layer)  # 得到注意力内容

        context_layer = context_layer.permute(0, 2, 1, 3).contiguous()
        new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,)  # 将head的维度还原
        context_layer = context_layer.view(*new_context_layer_shape)

        outputs = (context_layer, attention_probs) if output_attentions else (context_layer,)

        if self.is_decoder:
            outputs = outputs + (past_key_value,)  # gpt2中将过去的key和value追加
        return outputs
