...
def prepare_data():
    ...
    with open("./data/train_data.json", 'r', encoding='utf-8') as load_f:
        ...
        sub_train = info
    with open("./data/train-small.json", "w", encoding='utf-8') as dump_f:
        for i in sub_train:
            a = json.dumps(i, ensure_ascii=False)
            dump_f.write(a)
            dump_f.write("\n")
        # json.dump(dump_f, sub_train)
    ...
    with open("./data/dev_data.json", 'r', encoding='utf-8') as load_f:
        ...
        sub_train = info
    with open("./data/dev-small.json", "w", encoding='utf-8') as dump_f:
        for i in sub_train:
            a = json.dumps(i, ensure_ascii=False)
            dump_f.write(a)
            dump_f.write("\n")
...
def load_train():
    ...
    with open("./data/train-small.json", 'r', encoding='utf-8') as load_f:
...
def load_dev():
    ...
    with open("./data/dev-small.json", 'r', encoding='utf-8') as load_f:
        for line in load_f.readlines():
            dic = json.loads(line)
            if dic['rel'] not in rel2id:
                train_data['label'].append(0)
